...
```php
$typeSchema = SchemaFactory::string('name');
...
```php
$lengthSchema = SchemaFactory::string('username')
    ->minLength(3)
    ->maxLength(20);
...
```php
$requiredSchema = SchemaFactory::object('user')
    ->properties(
...
$userSchema = SchemaFactory::object('user')
    ->properties(
        SchemaFactory::string('name')->minLength(2)->required(),
            ...
            if (isset($schemaArray['properties'])) {
                foreach ($schemaArray['properties'] as $property => $propSchema) {
                    if (isset($data[$property])) {
                        ...
                        $propertySchema = SchemaFactory::fromJson($propSchema);
