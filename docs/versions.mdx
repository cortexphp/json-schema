...
<CardGroup cols={1}>
  <Card title="Draft-07 (2018)" icon="circle-check">
    **Default version** for maximum compatibility across tools and libraries. Includes core validation features and basic conditionals.
  </Card>
  ...
  <Card title="Draft 2020-12" icon="sparkles">
    **Latest version** with `prefixItems`, dynamic references, format vocabularies, and other cutting-edge features.
  </Card>
</CardGroup>
...
```php
// Set global default for all new schemas
SchemaFactory::setDefaultVersion(SchemaVersion::Draft_2019_09);

// All subsequent schemas will use Draft 2019-09 by default
$schema1 = SchemaFactory::string('field1'); // Uses Draft 2019-09
$schema2 = SchemaFactory::object('user');   // Uses Draft 2019-09
```
...
<Tip>
Start with Draft-07 for maximum compatibility, then upgrade to newer versions when you need advanced features like `unevaluatedProperties` or `dependentSchemas`.
</Tip>
