...
$priceSchema = SchemaFactory::number('price')
...
$temperatureSchema = SchemaFactory::number('temperature')
...
$percentageSchema = SchemaFactory::number('percentage')
...
$positiveSchema = SchemaFactory::number('value')
    ->exclusiveMinimum(0)
...
$ratingSchema = SchemaFactory::number('rating')
    ->minimum(0)
    ->maximum(5)
    ->multipleOf(0.5)
...
```php
// Discount percentage
$discountSchema = SchemaFactory::number('discount')
    ->minimum(0)
    ->maximum(100)
...
$weightSchema = SchemaFactory::number('weight')
...
$configSchema = SchemaFactory::object('config')
    ->properties(
        SchemaFactory::number('timeout')
            ->minimum(0.1)
            ->maximum(30)
            ->default(5.0)
            ->description('Request timeout in seconds'),

        SchemaFactory::number('retry_delay')
            ->minimum(0)
            ->default(1.0)
...
$orderSchema = SchemaFactory::object('order')
    ->properties(
        SchemaFactory::number('subtotal')
            ->minimum(0)
            ->multipleOf(0.01)
            ->required(),

        SchemaFactory::number('tax_rate')
            ->minimum(0)
            ->maximum(1)
            ->multipleOf(0.0001)
            ->required(),
...
$productSchema = SchemaFactory::object('product')
    ->properties(
        ...
        // Dimensions
        SchemaFactory::number('length')
            ->exclusiveMinimum(0)
            ->maximum(500)
            ->multipleOf(0.1)
            ->description('Length in centimeters'),

        SchemaFactory::number('width')
            ->exclusiveMinimum(0)
            ->maximum(500)
            ->multipleOf(0.1),

        SchemaFactory::number('height')
            ->exclusiveMinimum(0)
            ->maximum(500)
            ->multipleOf(0.1),

        // Rating
        SchemaFactory::number('average_rating')
            ->minimum(0)
            ->maximum(5)
            ->multipleOf(0.01)
            ->readOnly()
        ...
        // Discount
        SchemaFactory::number('discount_percentage')
            ->minimum(0)
...
$scientificSchema = SchemaFactory::number('measurement')
    ->description('Scientific measurement (may include infinity)');

// Most practical schemas should have bounds
$practicalSchema = SchemaFactory::number('practical_value')
    ->minimum(-1e6)
    ->maximum(1e6)
