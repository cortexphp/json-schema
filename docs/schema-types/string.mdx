...
Control the minimum and maximum length of strings:
...
$phoneSchema = SchemaFactory::string('phone')
...
// URL validation
$urlSchema = SchemaFactory::string('website')
...
```php
// Theme selection
$themeSchema = SchemaFactory::string('theme')
...
$statusSchema = SchemaFactory::string('status')
...
```php
$versionSchema = SchemaFactory::string('api_version')
    ->const('v1.0')
...
$idSchema = SchemaFactory::string('id')
    ->readOnly()
    ->pattern('^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$');

// Write-only field (e.g., password)
$passwordSchema = SchemaFactory::string('password')
    ->writeOnly()
    ->minLength(8)
    ->pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*\d)');
```
...
```php
$userSchema = SchemaFactory::object('user_profile')
    ->properties(
...
$legacyFieldSchema = SchemaFactory::string('old_field', SchemaVersion::Draft_2019_09)
    ->deprecated()
...
<CardGroup cols={2}>
  ...
  <Card
    title="String Formats Guide"
    icon="check-circle"
    href="/advanced/string-formats"
  >
