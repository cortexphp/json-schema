...
$activeSchema = SchemaFactory::boolean('active')
...
```php
$userPreferencesSchema = SchemaFactory::object('preferences')
    ->properties(
        SchemaFactory::boolean('email_notifications')
            ->default(true)
            ->description('Receive email notifications'),

        SchemaFactory::boolean('sms_notifications')
            ->default(false)
            ->description('Receive SMS notifications'),

        SchemaFactory::boolean('marketing_emails')
            ->default(false)
            ->description('Receive marketing emails'),

        SchemaFactory::boolean('dark_mode')
            ->default(false)
...
$registrationSchema = SchemaFactory::object('registration')
    ->properties(
        ...
        SchemaFactory::boolean('accept_privacy')
            ->writeOnly()
            ->required()
            ->description('Must accept privacy policy'),

        SchemaFactory::boolean('subscribe_newsletter')
            ->writeOnly()
            ->default(false)
...
$consentSchema = SchemaFactory::boolean('gdpr_consent')
    ->const(true)
...
$securityFlagSchema = SchemaFactory::boolean('is_compromised')
    ->const(false)
...
```php
$permissionsSchema = SchemaFactory::object('user_permissions')
    ->description('User access permissions')
    ->properties(
        // Basic permissions
        SchemaFactory::boolean('can_read')
            ->default(true)
            ->description('Can view content'),

        SchemaFactory::boolean('can_write')
            ->default(false)
            ->description('Can create/edit content'),

        SchemaFactory::boolean('can_delete')
            ->default(false)
            ->description('Can delete content'),

        // Admin permissions
        SchemaFactory::boolean('can_manage_users')
            ->default(false)
            ->description('Can manage other users'),

        SchemaFactory::boolean('can_access_admin')
            ->default(false)
            ->description('Can access admin panel'),

        // System permissions (read-only)
        SchemaFactory::boolean('is_admin')
            ->readOnly()
            ->description('Has administrator privileges'),

        SchemaFactory::boolean('is_owner')
            ->readOnly()
...
```php
$privacySchema = SchemaFactory::object('privacy_settings')
    ...
    ->properties(
        ...
        SchemaFactory::boolean('searchable')
            ->default(true)
            ->description('Allow profile to appear in search results'),

        SchemaFactory::boolean('show_online_status')
            ->default(true)
            ->description('Show when user is online'),

        // Contact preferences
        SchemaFactory::boolean('allow_friend_requests')
            ->default(true)
            ->description('Allow others to send friend requests'),

        SchemaFactory::boolean('allow_messages')
            ->default(true)
            ->description('Allow direct messages from others'),

        // Data usage
        SchemaFactory::boolean('analytics_consent')
            ->default(false)
            ->description('Consent to usage analytics'),

        SchemaFactory::boolean('personalization_consent')
            ->default(false)
...
$orderSchema = SchemaFactory::object('order')
    ->properties(
        ...
        SchemaFactory::string('gift_recipient_email')
    )
...
```php
$checklistSchema = SchemaFactory::array('checklist')
    ->items(SchemaFactory::boolean())
...
$namedChecklistSchema = SchemaFactory::object('task_completion')
    ->properties(
        SchemaFactory::boolean('task_1_completed')->default(false),
        SchemaFactory::boolean('task_2_completed')->default(false),
        SchemaFactory::boolean('task_3_completed')->default(false),
        SchemaFactory::boolean('all_tasks_reviewed')->default(false)
    );
```
...
$legacyFeatureSchema = SchemaFactory::boolean('old_feature', SchemaVersion::Draft_2019_09)
    ->deprecated()
    ->default(false)
...
```php
$schema = SchemaFactory::boolean('enabled');
...
```php
$booleanSchema = SchemaFactory::boolean('flag');
