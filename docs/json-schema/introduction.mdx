...
<Info>
All schema types support common properties like title, description, examples, default values, and metadata annotations. Empty `examples` lists are omitted from the output. Unknown or vendor-specific keywords (including OpenAPI `discriminator` and `x-*` extensions) can be attached with `keyword()`.
</Info>
...
- **Custom Keywords** - `keyword()` for unmodelled JSON Schema / OpenAPI keywords and `x-*` extensions on any schema, including nested ones
