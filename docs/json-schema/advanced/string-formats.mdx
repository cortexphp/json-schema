...
$idnEmailSchema = Schema::string('idn_email', SchemaVersion::Draft_2019_09)
...
```php
$iriSchema = Schema::string('iri', SchemaVersion::Draft_2019_09)
...
```php
$iriRefSchema = Schema::string('iri_ref', SchemaVersion::Draft_2019_09)
...
```php
$idnHostnameSchema = Schema::string('idn_hostname', SchemaVersion::Draft_2019_09)
...
```php
$jsonPointerSchema = Schema::string('json_pointer')
...
```php
$relativeJsonPointerSchema = Schema::string('relative_json_pointer')
...
```php
$uriTemplateSchema = Schema::string('uri_template')
...
$customEmailSchema = Schema::string('email')
    ->pattern('^[^@]+@[^@]+\.[^@]+$');  // Simplified, less robust

// Good: Use pattern for custom formats
$productCodeSchema = Schema::string('product_code')
...
$dateSchema = Schema::string('date')->format(SchemaFormat::Date);
