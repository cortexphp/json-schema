...
$coordinateSchema = Schema::array('coordinates')
    ->items(
        Schema::number()
            ->minimum(-180)
            ->maximum(180)
...
// Mixed type tuple
$recordSchema = Schema::array('record')
    ->items(
...
$prefixArraySchema = Schema::array('prefixed', SchemaVersion::Draft_2020_12)
    ->prefixItems([
        Schema::string(),   // First item must be string
        Schema::integer(),  // Second item must be integer
    ])
    ->items(Schema::boolean())  // Additional items must be boolean
