...
Repeated `properties()` calls **merge** (union) their arguments rather than replacing the previous list. The last schema for a given name wins. The same applies to `property()`.

```php
$schema = Schema::object('Relationship')
    ->properties(Schema::string('placeholder'))
    ->properties(Schema::string('real'));

// properties: placeholder and real
```
...
When the property key should not come from the schema title, use `property()` to set the name explicitly. Untitled nested schemas are allowed because the name is supplied separately. You can also mark properties as required in bulk with `requireProperties()`:
...
Use `properties()` when the constructor title *is* the property name. Use `property()` when you want to name a property independently of `title`, including anonymous inline schemas. `property()` is especially useful when converting from raw JSON Schema, where property keys are independent of any `title` metadata.

## Property Titles

Property names are defined by the schema constructor argument (or the name passed to `property()`), while `title()` is metadata and only included when it differs from the property name:
...
Specify which properties are mandatory. Mark a child schema with `required()`, name a list on the parent with `requireProperties()`, or require every defined property with `requireAll()`:
...
// Runtime-computed required lists — the counterpart to requireAll()
...
$schema = Schema::object('user')
    ...
    ->requireProperties(...$requiredFields);
