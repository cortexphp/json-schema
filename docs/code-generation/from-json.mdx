...
// Import from array
$productSchema = SchemaFactory::fromJson($schemaArray);
```
...
$modernSchema = SchemaFactory::fromJson($modern_schema_json);

// Or specify version explicitly
$explicitSchema = SchemaFactory::fromJson(
    $modern_schema_json,
...
$apiSchema = SchemaFactory::fromJson($complex_api_schema);
...
function loadSchemaFromFile(string $filepath): Schema
{
    ...
    return SchemaFactory::fromJson($schemaData);
}
...
class SchemaRegistry
{
    ...
    {
        $this->schemas[$name] = SchemaFactory::fromJson($jsonString);
    }
...
function extractOpenApiSchema(array $openApiSpec, string $componentName): Schema
{
    ...
    return SchemaFactory::fromJson($jsonSchema);
}
...
$modernSchema = SchemaFactory::fromJson($newSchema, SchemaVersion::Draft_2019_09);
```
...
function composeSchemas(array $schemaFiles, string $mainSchemaName): Schema
{
    ...
    return SchemaFactory::fromJson($composedSchema);
}
...
function safeJsonImport(string $jsonString): Schema
{
    ...
    return SchemaFactory::fromJson($data);
}
```
