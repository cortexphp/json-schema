...
$strictUserSchema = SchemaFactory::object('user', SchemaVersion::Draft_2019_09)
    ->properties(
...
```php
$flexibleUserSchema = SchemaFactory::object('user', SchemaVersion::Draft_2019_09)
    ->properties(
        SchemaFactory::string('name')->required(),
        SchemaFactory::string('email')->required()
    )
...
```php
$configSchema = SchemaFactory::object('config', SchemaVersion::Draft_2019_09)
    ->properties(
        SchemaFactory::string('app_name')->required(),
        SchemaFactory::string('version')->required()
    )
    ->patternProperties([
        '^env_' => SchemaFactory::string(),          // Environment variables
        '^feature_' => SchemaFactory::boolean(),     // Feature flags
    ])
...
$strictArraySchema = SchemaFactory::array('tags', SchemaVersion::Draft_2019_09)
    ->items(SchemaFactory::string())  // All items must be strings
...
```php
$mixedArraySchema = SchemaFactory::array('mixed_data', SchemaVersion::Draft_2019_09)
    ->items(
        SchemaFactory::string(),   // First item: string
        SchemaFactory::integer()   // Second item: integer
    )
    ->unevaluatedItems(
        SchemaFactory::boolean()   // Any additional items must be boolean
    )
...
```php
$conditionalUserSchema = SchemaFactory::object('user', SchemaVersion::Draft_2019_09)
    ->properties(
        SchemaFactory::string('type')->enum(['basic', 'premium', 'admin']),
        SchemaFactory::string('name')->required(),
        SchemaFactory::string('email')->required()
    )
    ...
    ->then(
        SchemaFactory::object()->properties(
            SchemaFactory::string('subscription_id')->required(),
            SchemaFactory::string('billing_cycle')->enum(['monthly', 'yearly'])
        )
    )
    ->if(
        SchemaFactory::object()->properties(
            SchemaFactory::string('type')->const('admin')
        )
    )
    ->then(
        SchemaFactory::object()->properties(
            SchemaFactory::array('permissions')
                ...
                ->required(),
            SchemaFactory::string('admin_level')->enum(['super', 'limited'])
        )
    )
...
```php
$complexSchema = SchemaFactory::object('user', SchemaVersion::Draft_2019_09)
    ->properties(
...
```php
$apiResponseSchema = SchemaFactory::object('api_response', SchemaVersion::Draft_2019_09)
    ->properties(
        SchemaFactory::boolean('success')->required(),
        SchemaFactory::string('message'),
        SchemaFactory::integer('status_code')->required()
    )
    // Success responses have data
    ->if(
        SchemaFactory::object()->properties(
            SchemaFactory::boolean('success')->const(true)
        )
    )
    ->then(
        SchemaFactory::object()->properties(
            SchemaFactory::union([SchemaType::Object, SchemaType::Array], 'data')
    ...
    ->if(
        SchemaFactory::object()->properties(
            SchemaFactory::boolean('success')->const(false)
        )
    )
    ->then(
        SchemaFactory::object()->properties(
            SchemaFactory::object('error')
                ->properties(
                    SchemaFactory::string('code')->required(),
                    SchemaFactory::string('details'),
                    SchemaFactory::array('validation_errors')
                        ->items(SchemaFactory::string())
                )
                ->required()
        )
...
$simpleSchema = SchemaFactory::object('simple')
    ->properties(
        SchemaFactory::string('name')->required()
    )
...
$complexSchema = SchemaFactory::object('complex', SchemaVersion::Draft_2019_09)
...
$schema = SchemaFactory::object('config', SchemaVersion::Draft_2019_09)
    ->properties(/* known properties */)
    ->patternProperties(/* pattern-based properties */)
    ->unevaluatedProperties(SchemaFactory::string());  // Validate remaining
```

### 3. Document the Behavior

```php
$schema = SchemaFactory::object('strict_user', SchemaVersion::Draft_2019_09)
