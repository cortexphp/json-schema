...
```php
$flexibleApiSchema = SchemaFactory::object('api_config')
    ->properties(
        SchemaFactory::string('name')->required(),
        SchemaFactory::string('version')->required()
    )
    ->patternProperties([
        '^header_' => SchemaFactory::string()
            ->description('HTTP headers'),

        '^param_' => SchemaFactory::union([SchemaType::String, SchemaType::Number])
            ->description('Query parameters'),

        '^feature_' => SchemaFactory::boolean()
            ->description('Feature flags'),

        '^\d+$' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('endpoint')->required(),
                SchemaFactory::string('method')->enum(['GET', 'POST', 'PUT', 'DELETE'])
            )
...
$dynamicFormSchema = SchemaFactory::object('dynamic_form')
    ->properties(
        SchemaFactory::string('form_id')->required(),
        SchemaFactory::string('title')->required()
    )
    ->patternProperties([
        // Text input fields
        '^field_text_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('type')->const('text')->required(),
                SchemaFactory::string('label')->required(),
                SchemaFactory::string('placeholder'),
                SchemaFactory::boolean('required')->default(false),
                SchemaFactory::integer('max_length')->minimum(1)->maximum(1000)
            )
            ->additionalProperties(false),

        // Number input fields
        '^field_number_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('type')->const('number')->required(),
                SchemaFactory::string('label')->required(),
                SchemaFactory::boolean('required')->default(false),
                SchemaFactory::number('min_value'),
                SchemaFactory::number('max_value'),
                SchemaFactory::number('step')->exclusiveMinimum(0)
            )
            ->additionalProperties(false),

        // Select/dropdown fields
        '^field_select_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('type')->const('select')->required(),
                SchemaFactory::string('label')->required(),
                SchemaFactory::boolean('required')->default(false),
                SchemaFactory::boolean('multiple')->default(false),
        ...
        '^validation_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('field_name')->required(),
                SchemaFactory::string('rule_type')
                    ->enum(['required', 'email', 'regex', 'min_length', 'max_length'])
                    ->required(),
                SchemaFactory::string('message')->required(),
                SchemaFactory::union([SchemaType::String, SchemaType::Number], 'parameter')
            )
            ->additionalProperties(false)
    ])
    ->additionalProperties(false);
...
$userProfileSchema = SchemaFactory::object('user_profile')
    ...
    ->properties(
        ...
        SchemaFactory::integer('age')->minimum(13)
    )
    // Pattern properties for custom fields
    ->patternProperties([
        '^custom_' => SchemaFactory::string()
            ->minLength(1)
            ->maxLength(255)
            ->description('Custom user fields'),

        '^social_' => SchemaFactory::string()
            ->format(SchemaFormat::Uri)
            ->description('Social media profiles'),

        '^pref_' => SchemaFactory::union([SchemaType::String, SchemaType::Boolean, SchemaType::Number])
            ->description('User preferences')
    ])
    // Allow additional string properties not matching patterns
    ->additionalProperties(SchemaFactory::string()->maxLength(100));
...
```php
$hierarchicalConfigSchema = SchemaFactory::object('hierarchical_config')
    ->patternProperties([
        // Environment-specific settings: dev_*, staging_*, prod_*
        '^(dev|staging|prod)_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('database_url')->required(),
                SchemaFactory::boolean('debug_mode')->required(),
                SchemaFactory::integer('max_connections')->minimum(1)
            ),

        // Service-specific settings: service_auth_*, service_cache_*, etc.
        '^service_[a-z]+_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::boolean('enabled')->required(),
                SchemaFactory::object('config')->additionalProperties(true)
            ),

        // Region-specific settings: region_us_*, region_eu_*, etc.
        '^region_[a-z]{2}_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('datacenter')->required(),
                SchemaFactory::array('availability_zones')
...
```php
$versionedApiSchema = SchemaFactory::object('versioned_api')
    ->properties(
        SchemaFactory::string('name')->required()
    )
    ->patternProperties([
        // Version-specific endpoints: v1_*, v2_*, v3_*
        '^v\d+_' => SchemaFactory::object()
            ->properties(
                SchemaFactory::string('path')->required(),
                SchemaFactory::string('method')->enum(['GET', 'POST', 'PUT', 'DELETE'])->required(),
                SchemaFactory::boolean('deprecated')->default(false),
                SchemaFactory::string('successor_version')
            )
            ->if(SchemaFactory::object()->properties(
                SchemaFactory::boolean('deprecated')->const(true)
            ))
            ->then(SchemaFactory::object()->properties(
                SchemaFactory::string('successor_version')->required()
            )),

        // Feature flags per version: feature_v1_*, feature_v2_*
        '^feature_v\d+_' => SchemaFactory::boolean()
...
```php
$localizationSchema = SchemaFactory::object('translations')
    ->patternProperties([
        // Language codes: en, es, fr, de, etc.
        '^[a-z]{2}(-[A-Z]{2})?$' => SchemaFactory::object()
            ->patternProperties([
                // Translation keys: button.save, error.validation.required, etc.
                '^[a-z]+(\.[a-z_]+)*$' => SchemaFactory::string()
                    ->minLength(1)
...
```php
$conditionalPatternSchema = SchemaFactory::object('conditional_config')
    ->properties(
        SchemaFactory::string('mode')->enum(['development', 'production'])->required()
    )
    ->patternProperties([
        '^debug_' => SchemaFactory::boolean()
            ->description('Debug settings')
    ])
    // Debug settings only allowed in development mode
    ->if(SchemaFactory::object()->properties(
        SchemaFactory::string('mode')->const('development')
    ))
    ->then(SchemaFactory::object()
        ->patternProperty('^debug_', SchemaFactory::boolean())
    )
    ->else(SchemaFactory::object()
        // In production, debug properties must be false or absent
        ->patternProperty('^debug_', SchemaFactory::boolean()->const(false))
    )
    ->additionalProperties(false);
```
...
```php
$prioritySchema = SchemaFactory::object('priority_example')
    ->properties(
        'exact_match' => SchemaFactory::string()->maxLength(10)  // 1. Exact match
    )
    ->patternProperty('^exact_',  // 2. Pattern match (won't match 'exact_match')
        SchemaFactory::string()->maxLength(20)
    )
    ->additionalProperties(  // 3. Anything else
        SchemaFactory::string()->maxLength(5)
    );
