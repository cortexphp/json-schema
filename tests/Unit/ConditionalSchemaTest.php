...
it('can create a schema with anyOf condition', function (): void {
    $objectSchema = Schema::object('payment')
        ->anyOf(
            Schema::object()->properties(
...
it('can create a schema with oneOf condition', function (): void {
    $objectSchema = Schema::object('contact')
        ->oneOf(
            Schema::object()->properties(
    ...
    expect(fn() => $objectSchema->validate([]))->toThrow(SchemaException::class);
});

it('can create a schema with not condition', function (): void {
    $stringSchema = Schema::string('status')->not(
